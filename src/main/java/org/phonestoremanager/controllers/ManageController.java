...
public class ManageController {
        public static Pane createManagerPane(ManageModel productViewModel) {
                Pane pane = new Pane();
                pane.setPrefSize(230, 330);
                ...
                try {
                        if (productViewModel.getImageUrl() != null) {
                                String imagePath = "/org/phonestoremanager/assets/image/" + productViewModel.getImageUrl();
                                Image image = new Image(ManageController.class.getResourceAsStream(imagePath));
                                if (image.isError()) {
                                        throw new Exception("Lỗi tải ảnh: " + productViewModel.getImageUrl());
                                }
                ...
                Label stockLabel = new Label("Tồn kho:");
                stockLabel.setStyle("-fx-font-size: 13px; -fx-text-fill: #555; -fx-font-weight: bold;");
                ...
                Label stockValue = new Label(String.valueOf(productViewModel.getInventory()));
                stockValue.setStyle("-fx-font-size: 14px; -fx-text-fill: #222;");

                gridPane.addRow(0, nameLabel, nameValue);
                gridPane.addRow(1, priceLabel, priceValue);
                gridPane.addRow(2, stockLabel, stockValue);
