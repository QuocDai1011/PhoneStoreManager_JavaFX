...
public class PdfGenerator {
    public static String createPdf(int index, String customerName, double total, List<OrderUpdateModel> list) {
        ...
        try {
            ...
            infoTable.addCell(createCell("Trạng thái đơn hàng:", normalFont, true));
            infoTable.addCell(createCell("ĐÃ THANH TOÁN", normalFont, false));
    ...
    public static String pdfCreateWithoutPayment(int index, String customerName, double total, List<OrderUpdateModel> list) {
        String filePath = "order-unpaid-" + index + ".pdf";
        String totalString = ParseVietNamCurrencyToDouble.formatToVietnamCurrency(total);
        ...
        try {
            ...
            // Tiêu đề
            Paragraph title = new Paragraph("HÓA ĐƠN CHỜ THANH TOÁN", titleFont);
            title.setAlignment(Element.ALIGN_CENTER);
            title.setSpacingAfter(10);
            document.add(title);
            ...
            infoTable.addCell(createCell("Trạng thái đơn hàng:", normalFont, true));
            infoTable.addCell(createCell("CHƯA THANH TOÁN", normalFont, false));
            ...
            // Thêm mã QR
            Paragraph qrTitle = new Paragraph("Vui lòng quét mã QR bên dưới để thanh toán:", normalFont);
            qrTitle.setSpacingBefore(20);
            qrTitle.setAlignment(Element.ALIGN_CENTER);
            document.add(qrTitle);

            Image qrImage = Image.getInstance("D:\\Coding\\JAVA\\BTL_JAVA\\src\\main\\resources\\org\\phonestoremanager\\assets\\image\\qr_pay.jpg");
            qrImage.scaleToFit(300, 300);
            qrImage.setAlignment(Element.ALIGN_CENTER);
            document.add(qrImage);

            Paragraph note = new Paragraph("Sau khi thanh toán, xin vui lòng giữ lại hóa đơn.", normalFont);
            note.setSpacingBefore(10);
            note.setAlignment(Element.ALIGN_CENTER);
            document.add(note);

            document.close();
            System.out.println("Tạo PDF chờ thanh toán thành công: " + filePath);
        } catch (Exception e) {
            e.printStackTrace();
        }
